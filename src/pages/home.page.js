...
export default class Home extends Component {
  ...
  componentDidMount() {
    var { windowWidth } = this.state;
    if (576 >= windowWidth) {
      this.setState({ NCards: 1 });
    }
    if ( 768 >= windowWidth > 576) {
      this.setState({ NCards: 2 });
    }
    if ( 992 >= windowWidth > 768) {
      this.setState({ NCards: 3 });
    }
    if ( 1200 >= windowWidth > 992) {
      this.setState({ NCards: 4 });
    }
    if ( windowWidth > 1200) {
      this.setState({ NCards: 5 });
    }
  }

  render() { 
    var { NCards, windowWidth } = this.state;
